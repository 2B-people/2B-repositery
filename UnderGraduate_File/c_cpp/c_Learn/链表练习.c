...
/*创建n个节点的链表
  ...
  struct student *Create()
  {
    ...
    struct student *p1=NULL;          //p1保存创立的新节点的地址
    struct student *p2=NULL;          //p2保存原链表的最后一个节点的地址
    n=0;    //创建的节点总数为0，空链表；
    p1=(struct student *)malloc(LEN);  //开辟新节点
    p2=p1;      //节点开辟成功，p2把指针保存下来；

    if(p1==NULL)   //节点没开辟成功
    {
        printf("\nCant creat it,try it again in a moment!\n");
        return NULL;
    }
    else            //节点开辟成功
    {
    ...
    while(p1->num!=0)//输入0表示结束输入
    {
        n+=1;     //节点个数加一
        if(n==1)  //如果节点总数是1，则head指向刚创建的节点p1
        {
            head=p1;
            p2->next=NULL; //此时的p2就是p1
        }
        else
        {
            p2->next=p1;   //指向上次开辟的新节点
        }
        p2=p1;  
